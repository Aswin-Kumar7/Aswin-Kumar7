...
#
# Repositories are NOT filtered to public: language totals include private repos
# too (so e.g. Dart-only private work still shows up). This means the token must
# be able to see those private repos — the default workflow GITHUB_TOKEN cannot,
# so the Action prefers a personal-access-token secret (STATS_TOKEN) when set.
# Without it, only public repos are counted and the numbers still render.
QUERY = """
...
def font_text():
    """Basic latin, both weights — for every graphic, headings included."""
...
def draw_heading(word):
    ...
    # The 600-weight face carries full basic latin, so any heading word renders
    # in JBMono (not only the handful the original head subset knew).
    p = [head(WIDTH, H, font=face("jbmono-600.woff2", 600))]
...
# Hackathon participations are not something the GitHub API knows about, so they
# live here as plain data and are drawn in the same visual language as the
# generated graphics. Edit this list; the card re-lays itself to fit.
# Fields: project (what you built), event (name of the hackathon), year, result.
HACKATHONS = [
    dict(project="RoadSOS",      event="IIT Madras · BIMSTEC Hackathon", year="2026", result="Winner · 1st"),
    dict(project="FactifyAI",    event="Innovate and Build Hackathon",        year="2025", result="Winner · 1st"),
    dict(project="Supply-Sense", event="Cognizant Technoverse",               year="2026", result="National Finalist"),
    dict(project="AuraAI",       event="AIM '25 Industry Hackathon",          year="2025", result="Runner up"),
    dict(project="traveloop",    event="Odoo x Kahe Hackathon",               year="2026", result="Finalist"),
    dict(project="transitops",   event="Odoo Hackathon",                      year="2026", result="Finalist"),
    dict(project="AIRO",         event="Kasparro Hackathon",                  year="2026", result="Participant"),
]


def draw_hackathons(events):
    """A card of hackathon participations, in the shared visual language.

    Not from any API — the events are declared in HACKATHONS above and drawn
    here so the section matches the generated stat graphics exactly: same grey
    ink, same monospace face, transparent background, staggered fade-in.
    """
    ROW = 46
    H = 14 + len(events) * ROW + 6
    p = [head(WIDTH, H)]
    for i, ev in enumerate(events):
        y0 = 14 + i * ROW
        delay = 0.10 + i * 0.09
        g = [f'<g opacity="0">{fade(delay)}']
        g.append(label(0, y0 + 17, f"{i + 1:02d}", 11, "m-f"))
        g.append(label(LEFT, y0 + 17, ev["project"], 15, "e-f",
                       extra=' font-weight="600"'))
        g.append(label(LEFT, y0 + 34, ev["event"], 11, "m-f"))
        g.append(label(WIDTH, y0 + 17, ev["result"], 12, "e-f", "end",
                       ' font-weight="600"'))
        g.append(label(WIDTH, y0 + 34, ev["year"], 10, "m-f", "end"))
        if i < len(events) - 1:
            ly = y0 + ROW - 6
            g.append(f'<line x1="0" y1="{ly:.1f}" x2="{WIDTH}" y2="{ly:.1f}" '
                     f'class="u-s" stroke-width="1"/>')
        g.append("</g>")
        p.append("".join(g))
    p.append("</svg>")
...
STATE_FILE = ".stats-state.json"


def signature(s):
    """A compact fingerprint of everything the graphics draw.

    If this is unchanged since the last run, there is nothing new to contribute
    and the graphics would be byte-identical — so we can skip regenerating.
    """
    return {
        "total": s["total"], "active": s["active"],
        "best_week": s["best_week"],
        "current": s["current"]["length"], "longest": s["longest"]["length"],
        "by_size": s["by_size"], "by_repo": s["by_repo"],
        "hackathons": HACKATHONS,
        "last_day": next((d["date"] for w in reversed(s["weeks"])
                          for d in reversed(w)
                          if d["contributionCount"] > 0), None),
    }
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "Aswin-Kumar7")
    ...
    # Check: has anything changed since the last daily run? If the fingerprint
    # matches and every output already exists, there are no new contributions
    # and nothing to redraw. If the check can't run (missing/unreadable state),
    # fall through and regenerate anyway — i.e. just refresh on the 24h cadence.
    outputs = ["stats.svg", "streak.svg", "langs.svg", "year.svg",
               "hackathons.svg"]
    outputs += [f"hd-{w.replace(' ', '-')}.svg" for w in
                ("about", "stack", "projects", "hackathons", "stats", "connect")]
    try:
        with open(state_path, encoding="utf-8") as f:
            prev = json.load(f)
        unchanged = prev == sig and all(
            os.path.exists(os.path.join(out_dir, n)) for n in outputs)
    except (OSError, ValueError):
        unchanged = False
    if unchanged:
        print(f"no new contributions since {sig['last_day']}; nothing to do")
        return

    files = {"stats.svg": draw_stats(s), "streak.svg": draw_streak(s),
             "langs.svg": draw_langs(s), "year.svg": draw_year(s),
             "hackathons.svg": draw_hackathons(HACKATHONS)}
    for word in ("about", "stack", "projects", "hackathons",
                 "stats", "connect"):
